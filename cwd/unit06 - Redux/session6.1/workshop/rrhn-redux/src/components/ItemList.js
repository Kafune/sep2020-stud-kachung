...
class ItemListUI extends React.Component {
  render() {
    ...
    return <div className="ItemList">
        ...
        <button id="markAsSeen" onClick={() => this.props.onMarkAsSeen(this.props.listSize)}>
...
function ListFooter(props) {
  return <footer id="ListFooter">visual design based on <a href="http://blog.trackduck.com/weekly/top-10-hacker-news-redesigns/unknown-author-2/">this redesign by unknown author</a>.</footer>
}

function Item(props) {
  let statusText;
  switch(props.status) {
    case "read": statusText = "already read"; break;
    case "seen": statusText = "seen before"; break;
    default:     statusText = "new"; 
  }
  let domain;
  if(props.item.url) {
    ...
    domain = <span className="domain">&emsp;({hostname}, {statusText})</span>
  }
  ...
  if( props.item.descendants !== undefined ) {
    let commentsUrl = 'https://news.ycombinator.com/item?id='+encodeURIComponent(props.item.id)
    comments = <a className="comments" href={commentsUrl} ><strong>{props.item.descendants}</strong> comments</a>
  }
  ...
  return (
    <div className={cssClasses}>
      <div className="mainInfo">
        <div>
          <a className="itemTitle" href={props.item.url} onClick={clickHandler}>{props.item.title}</a>{domain}
        </div>
        <div className="info">
